        ...
        UsedItem !== null && UsedItem.length !== 0 ?
          UsedItem.map((item,index) => (
            <div className="shadow-2xl rounded-md h-[400px] w-[250px] mx-auto flex flex-col justify-center items-center p-3 gap-2">
              ...
              <div className="bg-my_dark text-my_light py-2 rounded-md flex items-center justify-center uppercase w-[80%] cursor-pointer"
               onClick={()=>handleReuseItem(item,index)}>re use this item</div>
            </div>
          )) :
          <h2 className=" text-my_dark font-bold text-xl ">No Items To View</h2>
      }
    </div>
  )
}
